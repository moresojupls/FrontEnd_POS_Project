...
function MyOrderlist({setStateOrderlist}) {
  const [Orderlist,setOrderlist] = useState(window.location.pathname.split('/')[2] == 'product');
  useEffect(()=>{
    console.log('window.location.hostname : ',window.location.pathname.split('/')[2])
  },[])
    return (setStateOrderlist===false?<></>:<div className = 'd-flex ' style={{outlineStyle: 'auto', width:'20%', color:'black' ,background:'white'}}>
      <div style={{width:'100%'}}> 
  ...
  // useEffect(()=>{
  //   console.log("setStateOrderlist : ",setStateOrderlist)
  //   setOrderlist(setStateOrderlist)
  // },[setStateOrderlist ])
